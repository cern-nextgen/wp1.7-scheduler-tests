...
public:
    ...
    // Launches a graph: finds a free or creates a new T, then calls launchGraphDelegated on it with forwarded args
    template <typename... Args>
    void launchGraphDelegated(Args&&... args) {
        size_t idx = 0;
        {
            std::lock_guard<std::mutex> lock(m_mutex);
            if (!m_freeIndices.empty()) {
        ...
        // Call launchGraph outside the lock
        m_objects[idx]->launchGraphDelegated(std::forward<Args>(args)...);
